...
const Countdown = () => {
  const calculateTimeleft = () => {
    ...
    if (difference > 0) {
      timeLeft = {
        days: Math.floor(difference / (1000 * 60 * 60 * 24)).toLocaleString('ru', {
          minimumIntegerDigits: 2,
        }),
        hours: Math.floor((difference / (1000 * 60 * 60)) % 24).toLocaleString('ru', {
          minimumIntegerDigits: 2,
        }),
        minutes: Math.floor((difference / (1000 * 60)) % 60).toLocaleString('ru', {
          minimumIntegerDigits: 2,
        }),
        seconds: Math.floor((difference / 1000) % 60).toLocaleString('ru', {
          minimumIntegerDigits: 2,
        }),
      };
    }
